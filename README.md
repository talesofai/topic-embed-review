...
## 它做什么
拉某话题活动的内嵌页版本清单 → 下载各版本 OSS 产物 → 按上线红线静态审查(只读 / CSP / 外站资源 / 写接口 / 自绘宿主顶栏 / pushState / token 存储)→ 对比版本变更 → 出给 owner 的上线建议报告。**全程只读,不上线、不持 x-token。**
...
> 为什么够用:审查只需「各版本的页面产物」。版本清单走 `versions` 接口(dev-publish 令牌可调),产物在 OSS(公开读、拿到 url 即可 fetch)。**全程零 x-token、零写权限**——令牌即便泄露也只能读本活动版本清单。
